# TRIBE AID — Tribal Welfare Management Platform

TRIBE AID is a centralized tribal welfare management platform developed to connect tribal communities with volunteers, donors, NGOs and administrators through a digital ecosystem.

The platform focuses on improving transparency, accessibility and efficiency in welfare distribution by enabling aid requests, volunteer coordination, donation tracking, and administrative monitoring.

---

## Overview

Many tribal communities face challenges in accessing healthcare, education, welfare schemes, and emergency support due to geographical isolation and lack of centralized systems.

TRIBE AID aims to bridge this gap by providing a scalable web platform where tribal families can request support, volunteers can manage field activities, and administrators can monitor welfare operations in real time.

---

## Tech Stack

* Python, Django
* JavaScript
* HTML5 & CSS3
* Flutter, Dart
* mySQL
---


## Screenshots Web

Login Page

![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/login_page_website.jpeg)

Admin Dashboard

 ![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/admin_home.png)

 ![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/admin_add_district_off.png)

 ![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/admin_view_dist_off.png)

  ![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/admin_add_package.png)

  ![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/admin_view_package.png)

  ![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/admin_allocate_package.png)

  ![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/admin_view_allocated_pack.png)

District Officer Dashboard

  ![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/District_officer_dashboard.png)

Counsilor Dashboard

  ![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/councilor_dashboard.png)

## Screenshots Mobile

Login Page

![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/login_page_mobile.jpeg)

Coordinater Dashboard

![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/coordinater_home_page.jpeg)

Public View

![image alt](https://github.com/mahroof3734/TRIBE-AID/blob/main/public_view.jpeg)
